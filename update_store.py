# --- update_store.py (Nexus Bot - Windows Final) ---
...
# --- CONFIGURATION ---
# These paths must match the folder names you create!
KNOWLEDGE_SOURCES = {
    "essays": {
        "source_path": "essays",
...
# CRITICAL: This matches the model in bot.py
...
def process_single_source(source_name, source_info, processed_files):
    ...
    if not new_docs:
        print("Could not read new files.")
        return
    ...
    if chunked_docs:
        for i in range(0, len(chunked_docs), 50):
            ...
            if len(chunked_docs) > 50:
                 print(f"  - Processed batch {i // 50 + 1}... Pausing 10s for API safety...")
                 time.sleep(10)
